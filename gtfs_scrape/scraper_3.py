...
origin_dest2 = pd.read_csv('Mbta_stat_links.csv', sep=',')
...
def url_string(x,y):
    url = 'http://realtime.mbta.com/developer/api/v2.1/traveltimes?api_key=wX9NwuHnZU2ToO7GmGR9uw&format=json&from_stop={}&to_stop={}&from_datetime=1514437200&to_datetime=1514523600'.format(x,y)
    return url
...
for index, row in origin_dest2.iterrows():
    ...
    perf_df = perf_df.assign(origin_id = row["from_stop"], origin_name = row["from.stop.desc"], dest_id = row["to_stop"], dest_name = row["to.stop.desc"])
